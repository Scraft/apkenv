...
#ifdef APKENV_DEBUG
#  define WRAPPERS_DEBUG_PRINTF(_Fmt, ...) printf(_Fmt, ## __VA_ARGS__)
#else
...
{
    WRAPPERS_DEBUG_PRINTF("fgets(%p, %d, %p)\n", __s, __n, __stream);
    if (IS_STDIO_FILE(__stream))
...
my_floor(double __x)
{
    WRAPPERS_DEBUG_PRINTF("floor(%f)\n", __x);
    return floor(__x);
}
double
my_fmod(double __x, double __y)
{
    WRAPPERS_DEBUG_PRINTF("fmod(%f, %f)\n", __x, __y);
    return fmod(__x, __y);
}
FILE *
my_fopen(__const char *__restrict __filename, __const char *__restrict __modes)
{
    ...
    WRAPPERS_DEBUG_PRINTF("fopen(%s, %s) -> %p\n", __filename, __modes, f);
    return f;
}
int
my_fputc(int __c, FILE *__stream)
{
    WRAPPERS_DEBUG_PRINTF("fputc(%d, %p)\n", __c, __stream);
    if (IS_STDIO_FILE(__stream))
...
{
    WRAPPERS_DEBUG_PRINTF("fputs(%p, %p)\n", __s, __stream);
    if (IS_STDIO_FILE(__stream))
...
{
    WRAPPERS_DEBUG_PRINTF("fread(%p, %d, %d, %p)\n", __ptr, __size, __n, __stream);
...
{
    WRAPPERS_DEBUG_PRINTF("getaddrinfo(%p, %p, %p, %p)\n", __name, __service, __req, __pai);
...
{
    WRAPPERS_DEBUG_PRINTF("send(%d, %p, %d, %d)\n", __fd, __buf, __n, __flags);
...
{
    WRAPPERS_DEBUG_PRINTF("setvbuf(%p, %p, %d, %d)\n", __stream, __buf, __modes, __n);
    if (IS_STDIO_FILE(__stream)) {
...
my_sinh(double __x)
{
    WRAPPERS_DEBUG_PRINTF("sinh(%f)\n", __x);
    return sinh(__x);
}
int
my_socket(int __domain, int __type, int __protocol)
{
    WRAPPERS_DEBUG_PRINTF("socket(%d, %d, %d)\n", __domain, __type, __protocol);
    return socket(__domain, __type, __protocol);
}
double
my_sqrt(double __x)
{
    WRAPPERS_DEBUG_PRINTF("sqrt(%f)\n", __x);
    return sqrt(__x);
}
void
my_srand48(long int __seedval)
{
    WRAPPERS_DEBUG_PRINTF("srand48(%ld)\n", __seedval);
    srand48(__seedval);
}
int
...
my_tmpnam(char *__s)
{
    WRAPPERS_DEBUG_PRINTF("tmpnam(%p)\n", __s);
    //return tmpnam(__s);
    return NULL;
}
int
...
{
    WRAPPERS_DEBUG_PRINTF("my_fprintf(%p, %s)\n", stream, format);
    va_list ap;
    int result;
    va_start(ap, format);
...
{
    WRAPPERS_DEBUG_PRINTF("my_vfprintf(%p, %s)\n", stream, format);
    if (IS_STDIO_FILE(stream))
...
{
    WRAPPERS_DEBUG_PRINTF("my_fflush(%p)\n", stream);
    if (IS_STDIO_FILE(stream))
...
{
    WRAPPERS_DEBUG_PRINTF("sprintf( %s, %s )\n", str, format );
    va_list ap;
    va_start(ap, format);
